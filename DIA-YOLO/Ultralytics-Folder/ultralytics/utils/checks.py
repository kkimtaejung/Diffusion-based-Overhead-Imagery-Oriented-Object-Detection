...
def check_amp(model):
    """
    Checks AMP compatibility for the current YOLO model (supports custom 4-channel inputs).
    """
    ...
    if bool(pattern.search(gpu)):
        LOGGER.warning(f"{prefix}checks failed ❌. AMP training on {gpu} GPU may cause NaN losses or zero-mAP.")
        return False

    def amp_allclose(m):
        """Compare FP32 vs AMP predictions with 4-channel dummy image."""
        imgsz = max(256, int(model.stride.max() * 4))
        dummy = torch.randn(1, 4, imgsz, imgsz, device=device)  # ✅ 4채널
        a = m(dummy)[0].boxes.data  # FP32 inference
        with autocast(enabled=True):
            b = m(dummy)[0].boxes.data  # AMP inference
        return a.shape == b.shape and torch.allclose(a, b.float(), atol=0.5)

    LOGGER.info(f"{prefix}running Automatic Mixed Precision (AMP) checks on your model...")
    try:
        assert amp_allclose(model)
        LOGGER.info(f"{prefix}checks passed ✅")
    except AssertionError:
        LOGGER.warning(f"{prefix}checks failed ❌. AMP disabled to prevent instability.")
        return False
    except Exception as e:
        LOGGER.warning(f"{prefix}AMP check skipped due to error: {e}")
        return False
