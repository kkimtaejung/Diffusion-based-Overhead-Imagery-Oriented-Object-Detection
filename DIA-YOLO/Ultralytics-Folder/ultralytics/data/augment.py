...
class BaseTransform:
    ...
    def __call__(self, labels):
        if labels["img"].shape[2] == 3:
            alpha = np.ones((*labels["img"].shape[:2], 1), dtype=labels["img"].dtype) * 255
            labels["img"] = np.concatenate([labels["img"], alpha], axis=-1)
        """
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        if img.ndim == 4 and img.shape[0] == 1:
            img = img[0]  # 배치 제거
        if img.shape[0] in [1, 3, 4]:  # CHW 구조인 경우
            img = np.transpose(img, (1, 2, 0))  # → HWC 변환
            
        
        shape = img.shape[:2]  # current shape [height, width]
        
        # 수정: new_unpad 계산 전에 img 크기가 유효한지 체크
        if img.shape[0] == 0 or img.shape[1] == 0:
            raise ValueError("Input image has invalid dimensions: {}".format(img.shape))
        ...
        # ✅ 수정된 padding 코드
        img_padded = np.full((new_shape[1], new_shape[0], channels), 114, dtype=img.dtype)  # 임시 이미지 생성
        ...
        #print(f"[LetterBox] input shape: {img.shape}, new_unpad: {new_unpad}")
        ...
        # ✅ Resize safely for multi-channel image
        if shape[::-1] != new_unpad:
            if channels == 1:
                ...
                resized_channels = [
                    cv2.resize(img[:, :, c], new_unpad, interpolation=cv2.INTER_LINEAR)
        ...
        # ✅ Dynamic padding value based on channel count
        pad_value = (114,) * channels if channels > 1 else 114
