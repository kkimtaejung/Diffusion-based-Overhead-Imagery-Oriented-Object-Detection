...
class BaseDataset(Dataset):
    ...
    def load_image(self, i, rect_mode=True):
        """
        Load an image from dataset index 'i' and ensure it's 4-channel.
        ...
        Returns:
            im (np.ndarray): Loaded 4-channel image.
            (h0, w0) (tuple): Original image dimensions.
            (h, w) (tuple): Resized image dimensions.
        """
        ...
        if im is None:  # Not cached in RAM
            if fn.exists():  # load npy
                ...
                im = cv2.imread(f, cv2.IMREAD_UNCHANGED)  # ✅ Read with alpha or extra channels if present
            ...
            elif im.shape[2] == 3:  # BGR only
                h, w = im.shape[:2]
                alpha = np.zeros((h, w, 1), dtype=im.dtype)
                im = np.concatenate((im, alpha), axis=-1)  # Add empty 4th channel
            elif im.shape[2] > 4:
                im = im[:, :, :4]  # Use first 4 channels only if more than 4
        ...
        #print(f"[DEBUG] Loaded image {self.im_files[i]} shape: {im.shape}")
