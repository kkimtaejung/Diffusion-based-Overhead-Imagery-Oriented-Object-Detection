...
def read_image_from_tar(tar_obj, img_rel_path):
    #print('read_image_from_tar :', img_rel_path)

    # 경로 정규화
    norm_path = img_rel_path
    # 절대경로는 없앤다
    ...
    # os.path.normpath 적용(중복 슬래시 제거 등)
    ...
    for path in candidates:
        if path in tar_names:
            print("[DEBUG] Found in tar:", path)
            image_bytes = tar_obj.extractfile(path).read()
            return Image.open(io.BytesIO(image_bytes))

    # 실패 시 에러
    print("[ERROR] File not found! Tried:", candidates)
    raise FileNotFoundError(f"File {img_rel_path} not found in tar archive. Tried: {candidates}")
...
class BaseDepthDataset(Dataset):
    ...
    def _load_rgb_data(self, rgb_rel_path):
        #print('_load_rgb_data :',rgb_rel_path)
        # Read RGB data
    ...
    def _load_depth_data(self, depth_rel_path, filled_rel_path):
        #print('_load_depth_data :',depth_rel_path,filled_rel_path)
        # Read depth data
        outputs = {}
    ...
    def _read_image(self, img_rel_path) -> np.ndarray:
        if self.is_tar:
            ...
            # 경로 정규화 (tar 내부 경로와 정확히 맞추기)
            tar_path = img_rel_path
            while tar_path.startswith('/'):
                tar_path = tar_path[1:]
            while tar_path.startswith('./'):
                tar_path = tar_path[2:]
            tar_path = os.path.normpath(tar_path)
            #print(f"Attempting to read: {tar_path}")
            tar_names = self.tar_obj.getnames()
            if tar_path not in tar_names:
                print("[ERROR] Not found in tar! Tried:", tar_path)
                raise FileNotFoundError(f"File {tar_path} not found in tar archive.")
            image_to_read = self.tar_obj.extractfile(tar_path)
            ...
            if not os.path.exists(image_to_read):
                raise FileNotFoundError(f"File {img_rel_path} not found in directory.")
        image = Image.open(image_to_read)
        image = np.asarray(image)
        return image

    def _read_rgb_file(self, rel_path) -> np.ndarray:
        #print('_read_rgb_file :', rel_path)
        rgb = self._read_image(rel_path)

        # 💡 흑백 이미지인 경우 채널 차원 추가 (H, W) → (H, W, 3)
        if rgb.ndim == 2:  # grayscale
            rgb = np.stack([rgb] * 3, axis=-1)  # (H, W) → (H, W, 3)

        rgb = np.transpose(rgb, (2, 0, 1)).astype(int)  # (H, W, 3) → (3, H, W)
        return rgb

    def _read_depth_file(self, rel_path):
        #print('_read_depth_file :',rel_path)
